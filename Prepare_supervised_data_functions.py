...
def create_feature_engineering(df):
    ...
    df['Quarter_numeric'] = df['Quarter'].str.extract(r'(\d)').astype(int)
    ...
    df['YeshuvCrimeRate'] = df.groupby('Yeshuv')['Yeshuv'].transform('count')
    df['CrimeTrend'] = df.groupby('Year')['Year'].transform('count')

    # Adding interaction features: CrimeTrend multiplied by YeshuvCrimeRate
    df['CrimeTrend_CrimeRate'] = df['CrimeTrend'] * df['YeshuvCrimeRate']

    # Calculating average crime rate per police station
    station_crime_rate = df.groupby('PoliceStation')['YeshuvCrimeRate'].transform('mean')
    df['StationCrimeRateAvg'] = station_crime_rate

    # Calculating historical crime rate per Yeshuv
    df['YeshuvHistoricalCrimeRate'] = df.groupby('Yeshuv')['YeshuvCrimeRate'].transform('mean')

    # Calculating the number of nearby police stations within the same district
    df['StationsNearbyCount'] = df.groupby('PoliceDistrict')['PoliceStation'].transform('nunique')
    ...
    df['CityOrMoshav'] = df['Yeshuv'].apply(lambda x: 'City' if x in hebrew_cities else 'Moshav')
    
    return df

def get_preprocessor():
    """
    Function to create the preprocessing pipeline including imputation,
    scaling, and encoding.
    """
    
    # List of numeric and categorical columns
    numeric_features = ['Quarter_numeric', 'YeshuvCrimeRate', 'CrimeTrend',
                        'CrimeTrend_CrimeRate', 'StationCrimeRateAvg',
                        'YeshuvHistoricalCrimeRate', 'StationsNearbyCount']
    categorical_features = ['Yeshuv', 'PoliceStation', 'StatisticArea', 'PoliceMerhav', 'PoliceDistrict', 'CityOrMoshav']
    ...
    categorical_transformer = Pipeline(steps=[
        ...
        ('label_encoder', LabelEncoder())  # Label encode categorical features
    ])
